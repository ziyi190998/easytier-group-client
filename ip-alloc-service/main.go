// EasyTier 群友 IP 分配服务
// 部署于腾讯云：为群友客户端按机器标识（machine_id）原子分配虚拟 IP（随机挑选空闲位），
// 并下发组网参数（网络名/口令经 HTTPS 返回，口令不落客户端安装包）。
//
// 安全约定：
//   - 组网口令、管理密钥仅从环境变量读取；
//     邀请码机制已停用（codes.json 可选存在，仅用于旧记录备注，不再做任何校验）。
//   - 本服务不主动发起任何对外 HTTP 请求。
//   - 日志脱敏：不打印口令/管理密钥，机器标识与虚拟 IP 仅输出掩码形式。
package main
...
func loadConfig() (config, error) {
	...
	if c.poolEnd, err = netip.ParseAddr(envOr("IP_POOL_END", "10.144.0.200")); err != nil {
...
// ---------------------------------------------------------------------------
// 机器租约（邀请码机制已停用，machine_id 为唯一身份）
// ---------------------------------------------------------------------------
...
type allocServer struct {
	cfg       config
	mu        sync.Mutex
	codes     map[string]codeEntry // 旧邀请码备注表（可选，仅用于展示备注）
	pool      []string             // 全量可用 IP（有序）
	held      map[string]*lease    // ip -> lease
	byMachine map[string]string    // machine_id -> ip
}
...
func newAllocServer(cfg config) (*allocServer, error) {
	...
	s.loadCodes() // 可选：缺失不报错（匿名模式）
	return s, nil
}

// loadCodes 读取旧邀请码备注表（可选）。文件缺失或损坏均不视为错误。
func (s *allocServer) loadCodes() {
	raw, err := os.ReadFile(s.cfg.codesPath)
	if err != nil {
		log.Printf("未找到邀请码文件(%s)，以匿名模式运行（不做任何校验）", s.cfg.codesPath)
		return
	}
	var list []codeEntry
	if err := json.Unmarshal(raw, &list); err != nil {
		log.Printf("邀请码文件 JSON 无效，忽略: %v", err)
		return
	}
	codes := map[string]codeEntry{}
	for _, e := range list {
	...
	s.codes = codes
	s.mu.Unlock()
	log.Printf("已加载旧邀请码备注 %d 条（仅用于展示，不再校验）", len(codes))
}

// alloc 原子分配（随机挑空闲 IP）：
//   - 同机重复申请：幂等返回已持有 IP（重连场景）；
//   - 新机器：从未占用 IP 中随机挑一个。
func (s *allocServer) alloc(machineID, inviteCode string) (string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if ip, ok := s.byMachine[machineID]; ok {
		if l, live := s.held[ip]; live {
			l.LastHeartbeat = time.Now()
			return ip, nil
		}
		delete(s.byMachine, machineID) // 脏索引兜底
	}
	var free []string
	...
	if e, ok := s.codes[inviteCode]; ok && inviteCode != "" {
		note = e.Note // 旧客户端仍带邀请码时，用其备注便于管理员对照
	}
	s.held[ip] = &lease{MachineID: machineID, Note: note, IP: ip, LastHeartbeat: now, AllocatedAt: now}
	s.byMachine[machineID] = ip
	return ip, nil
}

func (s *allocServer) heartbeat(machineID, ip string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	l, ok := s.held[ip]
	if !ok || l.MachineID != machineID {
		return false
	}
	l.LastHeartbeat = time.Now()
	return true
...
func (s *allocServer) release(machineID, ip string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	l, ok := s.held[ip]
	if !ok || l.MachineID != machineID {
	...
	for range t.C {
		...
		for ip, l := range s.held {
			if now.Sub(l.LastHeartbeat) > s.cfg.heartbeatTTL {
				delete(s.held, ip)
				delete(s.byMachine, l.MachineID)
				log.Printf("租约超时释放: machine=%s ip=%s", maskMachine(l.MachineID), maskIP(ip))
			}
		}
		s.mu.Unlock()
	}
...
type allocReq struct {
	InviteCode string `json:"invite_code"` // 已停用：仅为兼容旧客户端，服务端忽略
	IP         string `json:"ip"`
...
func (s *allocServer) handleAlloc(w http.ResponseWriter, r *http.Request) {
	...
	ip, err := s.alloc(req.MachineID, req.InviteCode)
	if err != nil {
		log.Printf("alloc 池满: machine=%s", maskMachine(req.MachineID))
		writeErr(w, http.StatusServiceUnavailable, "pool_exhausted")
		return
	}
	log.Printf("alloc 成功: machine=%s ip=%s", maskMachine(req.MachineID), maskIP(ip))
...
func (s *allocServer) handleHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	if s.heartbeat(req.MachineID, req.IP) {
		writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
		return
	}
	log.Printf("heartbeat 失配: machine=%s ip=%s", maskMachine(req.MachineID), maskIP(req.IP))
...
func (s *allocServer) handleRelease(w http.ResponseWriter, r *http.Request) {
	...
	if s.release(req.MachineID, req.IP) {
		log.Printf("release 成功: machine=%s ip=%s", maskMachine(req.MachineID), maskIP(req.IP))
	}
	writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
}
